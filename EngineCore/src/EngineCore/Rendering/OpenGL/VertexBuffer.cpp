...
namespace EngineCore {
	...
	constexpr uint32_t shader_data_type_to_components_count(const ShaderDataType type) {
		switch (type) {
	...
	constexpr uint32_t shader_data_type_to_component_type(const ShaderDataType type) {
		switch (type) {
		case ShaderDataType::Float:
		case ShaderDataType::Float2:
		case ShaderDataType::Float3:
	...
	BufElement::BufElement(const ShaderDataType _type)
		:type(_type)
		,component_type(shader_data_type_to_component_type(_type))
		,components_cnt(shader_data_type_to_components_count(_type))
